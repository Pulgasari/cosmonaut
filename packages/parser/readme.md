...
2. Special higher-level parsing methods are stored in `cosmonaut/packages/parser/methods`. (They will be expanded and enhanced over time. They are kinda pure as well and could be used without the CosmonautParser we are building. But they are also the core parsing methodes the CosmonautParser provides.
...
- any comments in the code or defined error messages etc. are strictly using English
